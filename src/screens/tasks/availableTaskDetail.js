...
const availableTaskDetail = ({route, navigation}) => {
  ...
  const getAvailableTask = () => {
    ...
    fetch(
      ...
      .then(responseJson => {
        console.log('respone', responseJson[0]);
  ...
  const getAttachmentLink = value => {
    ...
    fetch(
      ...
      .then(response => response.json())
      .then(responseJson => {
        console.log('responeLink', responseJson);
  ...
  const updateActivities = () => {
    ...
    .then(response => response.json())
      .then(responseJson => {
        console.log("respones",responseJson)
        setLoader(false);
              ...
              renderItem={({item, index}) => (
                <View
                  ...
                  <View style={styles.orderDetailServicesListHeader}>
                    <TouchableOpacity style={[styles.orderDetailRatingView , {width: '80%'}]}>
                      <Octicons
